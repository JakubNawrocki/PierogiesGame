...
export default class GameScene extends Phaser.Scene {
    ...
    preload() {
        ...
        // FIXED: Consistent obstacle texture key naming
    ...
    async create() { // Make create async if you await holder status
        ...
        const groundSprite = this.ground.create(this.scale.width / 2, this.scale.height - 40, TextureKeys.GroundKitchen) as Phaser.Physics.Arcade.Sprite;
        ...
        // FIXED: Check if player has a body before setting drag
        if (this.player.body) {
    ...
    update(time: number, delta: number) {
        ...
        if (this.player.isTrulyDead() && this.player.body && this.player.body.onFloor()) {
        ...
        // FIXED: Add null check for player body
        if (this.player.body) {
    ...
    private spawnObstacle() {
        ...
        // FIXED: Use consistent texture key names that match what we loaded
        const obstacleTypes = [
            TextureKeys.ObstacleKnife, 
            TextureKeys.ObstacleKielbasa, 
            TextureKeys.ObstacleOnion,
            TextureKeys.ObstacleGrater, 
            TextureKeys.ObstacleRollingPin
        ];
        ...
        switch(randomType) {
            case TextureKeys.ObstacleKnife: 
                obsWidth = 30; 
                obsHeight = Phaser.Math.Between(80, 200); 
                yPos = Phaser.Math.Between(this.scale.height - 80 - obsHeight, this.scale.height - 80); 
                break;
            case TextureKeys.ObstacleKielbasa: 
                obsWidth = 100; 
                obsHeight = 40; 
                yPos = Phaser.Math.Between(100, this.scale.height - 100); 
                break;
            case TextureKeys.ObstacleOnion: 
                obsWidth = 50; 
                obsHeight = 50; 
                yPos = Phaser.Math.Between(this.scale.height - 80 - obsHeight, this.scale.height - 80); 
                break;
            case TextureKeys.ObstacleGrater: 
                obsWidth = 60; 
                obsHeight = Phaser.Math.Between(120, 250); 
                yPos = this.scale.height - 40 - obsHeight / 2; 
                break; // Stands on ground
            case TextureKeys.ObstacleRollingPin: 
                obsWidth = 120; 
                obsHeight = 30; 
                yPos = Phaser.Math.RND.pick([150, this.scale.height / 2, this.scale.height - 150]); 
                break; // Can appear at different heights
        }
